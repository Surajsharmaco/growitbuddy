...
globalThis.__filename = globalThis.__filename || __fup(import.meta.url);
globalThis.__dirname = globalThis.__dirname || __path.dirname(globalThis.__filename);
`,
  },
}).then(() => {
  console.log(`\n✓ Vercel API function bundled → ${outfile}\n`);
}).catch((err) => {
  console.error(err);
  process.exit(1);
});
